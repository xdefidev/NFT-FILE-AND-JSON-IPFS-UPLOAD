...
import Body from "../connectwallet/connectwallet";
...
export default class UploadToIPFS extends Component {
  constructor(props) {
    ...
    this.state = {
      jsonInput: {
        name: "SPEED",
        description:
          "Speed bank presenting unique NFTS.  Speed ATM and Speed currency.",
        image: "",
        image_4K: "",
        attributes: {
          bunnyId: "1",
    ...
    if (this.state.buffer !== null) {
      ...
      this.setState((prevState) => ({
        jsonInput: {
          ...prevState.jsonInput,
          image: `ipfs://${cid.path}`,
          image_4K: `ipfs://${cid.path}`
        }
      }));
      this.setState({
        fileIPFSLink: `https://ipfs.infura.io/ipfs/${cid.path}`
      });
  ...
  uploadJSONToIPFS = async (e) => {
    ...
    this.setState({ jsonIPFSLink: `ipfs://${cid.path}` });
          ...
          <div style={{ marginTop: "5rem" }}>
            ...
            <div className="mt-1">
              <form onSubmit={this.uploadJSONToIPFS}>
                ...
                <Body
                  URI={this.state.jsonIPFSLink}
                  bunnyId={this.state.jsonInput.attributes.bunnyId}
                />
              </form>
            </div>
          </div>
